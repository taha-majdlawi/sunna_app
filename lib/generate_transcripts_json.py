...
# القائمة الكاملة والمنقحة (بدون تكرارات)
raw_data_input = """
...
# 1. بناء قاموس العناوين الذكي
titles_map = {}
lines = raw_data_input.strip().split('\n')

for line in lines:
    clean_line = normalize_to_english(line.strip())
    # استخراج رقم الحلقة من بين القوسين
    match = re.search(r'\( ?(\d+) ?\)', clean_line)
    if match:
        num = match.group(1).zfill(3)
        # استخراج العنوان (الجزء الذي يسبق النقاط أو كلمة السيرة)
        title_part = clean_line.split('..')[0].split('(')[0]
        title_part = re.sub(r'^\d+\.\s*', '', title_part).strip()
        
        # إذا كان العنوان مفقوداً (مثل الحلقات 71-201)
        if not title_part or title_part.lower() == "السيرة النبوية":
             title_part = f"السيرة النبوية - الحلقة {int(num)}"
             
        titles_map[num] = title_part

# 2. فحص المجلد وربط الملفات الموجودة
final_json_data = []
if os.path.exists(project_assets_dir):
    ...
    for f_name in files:
        ...
        # جلب العنوان من القاموس أو وضع عنوان افتراضي إذا لم يوجد
...
# 3. حفظ ملف JSON النهائي
with open(json_output_path, "w", encoding="utf-8") as f:
    json.dump(final_json_data, f, ensure_ascii=False, indent=4)

print("-" * 50)
print(f"✅ تم بنجاح! تم ربط {len(final_json_data)} حلقة بعناوينها.")
print(f"📍 الملف الجديد جاهز في: {json_output_path}")
print("-" * 50)